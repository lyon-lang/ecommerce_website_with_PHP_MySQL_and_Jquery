...
$ids = implode(',',$idArray);
$productsQ = $db->query("SELECT i.id as 'id', i.title as 'title', c.id as 'cid', c.category as 'child', p.category as 'parent'
 FROM products i 
 LEFT JOIN categories c ON i.categories = c.id 
 LEFT JOIN categories p ON c.parent = p.id
 WHERE i.id IN ({$ids})
 ");
 while($p = mysqli_fetch_assoc($productsQ)){
     foreach($items as $item){
         if($item['id'] == $p['id']){
             $x = $item;
             continue;
