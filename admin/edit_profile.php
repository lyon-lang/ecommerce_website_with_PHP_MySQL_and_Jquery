...
$userid = $user_data['id'];
if($_POST){
	$productresults = $db->query("SELECT * FROM users WHERE id = '$userid'");
	$userquery = mysqli_fetch_assoc($productresults);
			 ...
			 $insertSql = "UPDATE users SET profile_image = '$dbPath', bio = '$bio'
			 WHERE id = '$userid'";
				}else{
					$insertSql = "UPDATE users SET bio = '$bio'
			 WHERE id = '$userid'";
				}
		$db->query($insertSql);
...
if(isset($_GET['delete_image'])){
	...
	$db->query("UPDATE users SET profile_image = '{$imageString}' WHERE id = $userid");
