...
$userid = $cuser_data['id'];
if($_POST){
	$productresults = $db->query("SELECT * FROM customers WHERE id = '$userid'");
	$userquery = mysqli_fetch_assoc($productresults);
			 ...
			 $insertSql = "UPDATE customers SET profile_image = '$dbPath', bio = '$bio'
			 WHERE id = '$userid'";
				}else{
					$insertSql = "UPDATE customers SET bio = '$bio'
			 WHERE id = '$userid'";
				}
		$db->query($insertSql);
...
if(isset($_GET['delete_image'])){
	...
	$db->query("UPDATE customers SET profile_image = '{$imageString}' WHERE id = $userid");
